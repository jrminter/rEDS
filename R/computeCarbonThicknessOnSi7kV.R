#' Compute the C thickness on Si at 7kV
#'
#' Use the results from a LOESS fit to results from DTSA-II Monte Carlo
#' simulations of C thicknesses between 0 and 100 nm on Si at 7 kV
#'
#' @param c.to.si Ratio of C to Si peak intensities at 7 kV.
#'
...
#'
#' library(rEDS)
#' a <- computeCarbonThicknessOnSi7kV(0.2)
#' print(a)
#'
#' @export
computeCarbonThicknessOnSi7kV <- function(c.to.si, verbose=FALSE){
  t.C.nm <- seq(from=0.5, to=100, by=0.5)
  df2 <- data.frame(t.C.nm=t.C.nm, c.to.si.mu=t.C.nm)
  fi <- system.file("extdata", "c-to-si-loess-7kV.rda", package = "rEDS")
  load(fi)
  if(verbose) print(ls())
